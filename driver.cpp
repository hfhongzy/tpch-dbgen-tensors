...
#define NO_LFUNC (long (*) (int, long int)) NULL		/* to clean up tdefs */
...
tdef tdefs[] =
{
	{"part.tbl", "part table", 200000, NULL,
		{(int (*)(void*, int))pr_part, NULL}, sd_part, (int (*)(void*, int)) vrf_part, PSUPP, 0},

	{"partsupp.tbl", "partsupplier table", 200000, NULL,
		{(int (*)(void*, int))pr_psupp, NULL}, sd_psupp, (int (*)(void*, int)) vrf_psupp, NONE, 0},

	{"supplier.tbl", "suppliers table", 10000, NULL,
	 {(int (*)(void*, int))pr_supp, NULL}, sd_supp, (int (*)(void*, int)) vrf_supp, NONE, 0},


	{"customer.tbl", "customers table", 150000, NULL,
		{(int (*)(void*, int))pr_cust, NULL}, sd_cust, (int (*)(void*, int)) vrf_cust, NONE, 0},

	{"orders.tbl", "order table", 150000, NULL,
		{(int (*)(void*, int))pr_order, NULL}, sd_order, (int (*)(void*, int)) vrf_order, LINE, 0},

	{"lineitem.tbl", "lineitem table", 150000, NULL,
		{(int (*)(void*, int))pr_line, NULL}, sd_line, (int (*)(void*, int)) vrf_line, NONE, 0},

	{"orders.tbl", "orders/lineitem tables", 150000, NULL,
		{(int (*)(void*, int))pr_order_line, NULL}, sd_order, (int (*)(void*, int)) vrf_order_line, LINE, 0},

	{"part.tbl", "part/partsupplier tables", 200000, NULL,
		{(int (*)(void*, int))pr_part_psupp, NULL}, sd_part, (int (*)(void*, int)) vrf_part_psupp, PSUPP, 0},

	{"nation.tbl", "nation table", NATIONS_MAX, NULL,
		{(int (*)(void*, int))pr_nation, NULL}, NO_LFUNC, (int (*)(void*, int))vrf_nation, NONE, 0},

	{"region.tbl", "region table", NATIONS_MAX, NULL,
		{(int (*)(void*, int))pr_region, NULL}, NO_LFUNC, (int (*)(void*, int))vrf_region, NONE, 0}
...
// We only support start = 1, count = rows, upd_num = 1
/*
#define  PART		0
#define  PSUPP		1
#define  SUPP		2 (o)
#define  CUST		3 (o)
#define  ORDER		4
#define  ORDER_LINE     6 (o)
#define  PART_PSUPP     7 (o)
#define  NATION		8 (o)
#define  REGION		9 (o)
#define  UPDATE		10
#define  MAX_TABLE	11
...
gen_tbl (int tnum, long start, long count, long upd_num)
{
	...
	if (tnum == PART_PSUPP) {
		pre_pr_part(count);
		pre_pr_psupp(count * SUPP_PER_PART);
	}
	if (tnum == CUST) {
		pre_pr_cust(count);
	}
...
int main (int ac, char **av)
{
	...
	table = (1 << CUST) |
		...
		(1 << PART_PSUPP) |
		(1 << ORDER_LINE);
	force = 1; // overwrite by default
	printf ("It will overwrite existing files. Enter 1 to continue.\n");
	if (scanf ("%d", &force) != 1 || force != 1)
	{
		fprintf (stderr, "Aborting load.\n");
		exit (1);
	}
	insert_segments=0; //no
	delete_segments=0;
	insert_orders_segment=0;
	insert_lineitem_segment=0;
	delete_segment=0;
	verbose = 1;  // set to 1 by default
	columnar = 0;
	set_seeds = 0;
	header = 0;
	direct = 0;
			...
			if (children > 1 && i < NATION)
				...
				else
				{
					...
					if (verbose > 0)
						...
						"Generating", tdefs[i].comment, DSS_PROC);
					
					fprintf (stderr, "\ni = %d, minrows: %ld, rowcnt: %ld, upd_num: %ld\n", i, minrow, rowcnt, upd_num + 1);
